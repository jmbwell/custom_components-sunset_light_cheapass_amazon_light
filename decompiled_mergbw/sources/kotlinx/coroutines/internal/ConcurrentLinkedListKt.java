...
public final class ConcurrentLinkedListKt {
    ...
    public static final <S extends Segment<S>> Object findSegmentInternal(S s, long j, Function2<? super Long, ? super S, ? extends S> function2) {
        while (true) {
            if (s.id >= j && !s.isRemoved()) {
                return SegmentOrClosed.m2653constructorimpl(s);
            }
            Object nextOrClosed = s.getNextOrClosed();
            if (nextOrClosed == CLOSED) {
                return SegmentOrClosed.m2653constructorimpl(CLOSED);
            }
    ...
    public static final /* synthetic */ <S extends Segment<S>> Object findSegmentAndMoveForward$atomicfu(Object obj, AtomicReferenceFieldUpdater atomicReferenceFieldUpdater, long j, S s, Function2<? super Long, ? super S, ? extends S> function2) {
        ...
        loop0: while (true) {
            ...
            if (!SegmentOrClosed.m2658isClosedimpl(findSegmentInternal)) {
                Segment m2656getSegmentimpl = SegmentOrClosed.m2656getSegmentimpl(findSegmentInternal);
                while (true) {
                    Segment segment = (Segment) atomicReferenceFieldUpdater.get(obj);
                    if (segment.id >= m2656getSegmentimpl.id) {
                        break loop0;
                    } else if (!m2656getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        break;
                    } else if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(atomicReferenceFieldUpdater, obj, segment, m2656getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                    } else if (m2656getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        m2656getSegmentimpl.remove();
                    }
                }
            } else {
                break;
            ...
            if (!SegmentOrClosed.m2658isClosedimpl(findSegmentInternal)) {
                ...
                while (true) {
                    ...
                    if (segment.id >= m2656getSegmentimpl.id) {
                        ...
                        break;
                    } else if (Striped$SmallLazyStriped$$ExternalSyntheticBackportWithForwarding0.m(atomicReferenceArray, i, segment, m2656getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                    } else if (m2656getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
