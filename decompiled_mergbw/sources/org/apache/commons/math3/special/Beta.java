        ...
        return (FastMath.exp((((FastMath.log(d) * d2) + (FastMath.log1p(-d) * d3)) - FastMath.log(d2)) - logBeta(d2, d3)) * 1.0d) / new ContinuedFraction() { // from class: org.apache.commons.math3.special.Beta.1
            ...
            protected double getB(int i2, double d7) {
                if (i2 % 2 == 0) {
                    ...
                    double d9 = d2;
                    double d10 = d8 * 2.0d;
                    return (((d3 - d8) * d8) * d7) / (((d9 + d10) - 1.0d) * (d9 + d10));
                }
                double d11 = (i2 - 1.0d) / 2.0d;
                double d12 = d2;
                double d13 = d11 * 2.0d;
                return (-(((d12 + d11) * ((d3 + d12) + d11)) * d7)) / ((d12 + d13) * ((d12 + d13) + 1.0d));
            }
        }.evaluate(d, d4, i);
    }
        ...
        if (min >= 10.0d) {
            ...
            if (min < 1.0d) {
                ...
                if (max < 10.0d) {
                    ...
                    return FastMath.log(d4) + Gamma.logGamma(min) + (Gamma.logGamma(max) - logGammaSum(min, max));
                }
                return Gamma.logGamma(min) + logGammaMinusLogGammaSum(min, max);
            } else {
            ...
            return (FastMath.log(d5) - (floor * FastMath.log(max))) + Gamma.logGamma(min) + logGammaMinusLogGammaSum(min, max);
        } else {
            double d6 = 1.0d;
            while (min > 2.0d) {
                min -= 1.0d;
            ...
            if (max < 10.0d) {
                ...
                return FastMath.log(d6) + FastMath.log(d8) + Gamma.logGamma(min) + (Gamma.logGamma(max) - logGammaSum(min, max));
            }
            return FastMath.log(d6) + Gamma.logGamma(min) + logGammaMinusLogGammaSum(min, max);
        }
    }
}
