...
public class Gamma {
    ...
    public static double logGamma(double d) {
        ...
        if (d <= 8.0d) {
            ...
            return logGamma1p(d - (floor + 1)) + FastMath.log(d2);
        }
    ...
    public static double regularizedGammaP(double d, double d2, double d3, int i) {
        if (!Double.isNaN(d) && !Double.isNaN(d2)) {
            ...
            if (d > 0.0d && d2 >= 0.0d) {
                ...
                double d5 = 1.0d / d;
                double d6 = d5;
                while (FastMath.abs(d5 / d6) > d3 && d4 < i && d6 < Double.POSITIVE_INFINITY) {
                    d4 += 1.0d;
                    d5 *= d2 / (d + d4);
                    d6 += d5;
                }
                ...
                return FastMath.exp(((-d2) + (FastMath.log(d2) * d)) - logGamma(d)) * d6;
            }
        }
        return Double.NaN;
    }
    ...
    public static double regularizedGammaQ(final double d, double d2, double d3, int i) {
        ...
        return (1.0d / new ContinuedFraction() { // from class: org.apache.commons.math3.special.Gamma.1
    ...
    public static double gamma(double d) {
        if (d != FastMath.rint(d) || d > 0.0d) {
            ...
            if (abs > 20.0d) {
                ...
                double lanczos = lanczos(abs) * (SQRT_TWO_PI / d) * FastMath.pow(d2, 0.5d + abs) * FastMath.exp(-d2);
