...
public class CurveFitter<T extends ParametricUnivariateFunction> {
    ...
    public double[] fit(int i, T t, double[] dArr) {
        ...
        if (multivariateDifferentiableVectorOptimizer == null) {
            optimize = this.oldOptimizer.optimize(i, new OldTheoreticalValuesFunction(t), dArr2, dArr3, dArr);
        } else {
            optimize = multivariateDifferentiableVectorOptimizer.optimize(i, new TheoreticalValuesFunction(t), dArr2, dArr3, dArr);
        }
    ...
    public class OldTheoreticalValuesFunction implements DifferentiableMultivariateVectorFunction {
            ...
            return new MultivariateMatrixFunction() { // from class: org.apache.commons.math3.optimization.fitting.CurveFitter.OldTheoreticalValuesFunction.1
                ...
                public double[][] value(double[] dArr) {
                    ...
                    for (WeightedObservedPoint weightedObservedPoint : CurveFitter.this.observations) {
                        dArr2[i] = OldTheoreticalValuesFunction.this.f.gradient(weightedObservedPoint.getX(), dArr);
                        i++;
                    }
                    return dArr2;
                }
        ...
        public double[] value(double[] dArr) {
            ...
            int i = 0;
            for (WeightedObservedPoint weightedObservedPoint : CurveFitter.this.observations) {
                dArr2[i] = this.f.value(weightedObservedPoint.getX(), dArr);
                i++;
            }
            return dArr2;
        }
    ...
    public class TheoreticalValuesFunction implements MultivariateDifferentiableVectorFunction {
        ...
        public DerivativeStructure[] value(DerivativeStructure[] derivativeStructureArr) {
            ...
            int i2 = 0;
            for (WeightedObservedPoint weightedObservedPoint : CurveFitter.this.observations) {
                DerivativeStructure derivativeStructure = new DerivativeStructure(derivativeStructureArr.length, 1, this.f.value(weightedObservedPoint.getX(), dArr));
