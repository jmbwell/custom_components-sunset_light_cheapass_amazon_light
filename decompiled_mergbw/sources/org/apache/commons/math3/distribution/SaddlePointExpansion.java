...
final class SaddlePointExpansion {
    ...
    public static double getStirlingError(double d) {
        if (d >= 15.0d) {
            double d2 = d * d;
            return (0.08333333333333333d - ((0.002777777777777778d - ((7.936507936507937E-4d - ((5.952380952380953E-4d - (8.417508417508417E-4d / d2)) / d2)) / d2)) / d2)) / d;
        }
        double d3 = 2.0d * d;
        if (FastMath.floor(d3) == d3) {
            return EXACT_STIRLING_ERRORS[(int) d3];
        }
        return ((Gamma.logGamma(1.0d + d) - ((0.5d + d) * FastMath.log(d))) + d) - HALF_LOG_2_PI;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public static double getDeviancePart(double d, double d2) {
        double d3 = d - d2;
        double d4 = d + d2;
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public static double logBinomialProbability(int i, int i2, double d, double d2) {
        double d3;
        double log;
        if (i == 0) {
            if (d < 0.1d) {
