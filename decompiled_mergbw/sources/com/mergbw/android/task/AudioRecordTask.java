...
public class AudioRecordTask implements Runnable {
    ...
    public AudioRecordTask(int i, float f, IAudioRecordListener iAudioRecordListener) {
        ...
        int minBufferSize = AudioRecord.getMinBufferSize(sampleRateInHz, 16, 3);
        this.mAudioBufferSize = minBufferSize;
        this.mAudioRecord = new AudioRecord(1, sampleRateInHz, 16, 3, minBufferSize);
        this.mFFT = new DoubleFFT_1D(640L);
    }
    ...
    public void run() {
        while (this.isRecording) {
            if (this.mAudioRecord == null) {
                this.mAudioRecord = new AudioRecord(1, sampleRateInHz, 16, 3, this.mAudioBufferSize);
            }
    ...
    private void toHSV(byte[] bArr) {
        ...
        float f2 = 0.0f;
        for (byte b2 : bArr) {
            float abs = Math.abs((int) b2);
            f += abs;
            f2 = Math.max(f2, abs);
        }
        float length = (f2 - (f / bArr.length)) / f2;
        float f3 = ((double) length) >= ((double) (100 - this.mAudioSensitive)) * 1.7E-4d ? 5.0f * length : 0.0f;
        ...
        float abs2 = (float) (Math.abs(((d / 320) - d4) / (d5 - d4)) * 10.0d);
        Log.i(TAG, "toHSV: " + abs2 + " " + f3);
