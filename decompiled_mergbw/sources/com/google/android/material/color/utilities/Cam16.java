...
public final class Cam16 {
    ...
    public static Cam16 fromXyzInViewingConditions(double d, double d2, double d3, ViewingConditions viewingConditions) {
        ...
        double c2 = (4.0d / viewingConditions.getC()) * Math.sqrt(d15) * (viewingConditions.getAw() + 4.0d) * viewingConditions.getFlRoot();
        ...
        return new Cam16(d14, sqrt, pow4, c2, flRoot, Math.sqrt((pow5 * viewingConditions.getC()) / (viewingConditions.getAw() + 4.0d)) * 50.0d, (1.7000000000000002d * pow4) / ((0.007d * pow4) + 1.0d), log1p * Math.cos(radians), log1p * Math.sin(radians));
    }
    ...
    private static Cam16 fromJchInViewingConditions(double d, double d2, double d3, ViewingConditions viewingConditions) {
        ...
        return new Cam16(d3, d2, d, c2, flRoot, Math.sqrt(((d2 / Math.sqrt(d4)) * viewingConditions.getC()) / (viewingConditions.getAw() + 4.0d)) * 50.0d, d5, Math.cos(radians) * log1p, Math.sin(radians) * log1p);
    }
