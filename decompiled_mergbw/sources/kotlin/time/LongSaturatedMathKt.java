...
public final class LongSaturatedMathKt {
    ...
    public static final long m2532saturatingAddNuflL3o(long j, DurationUnit unit, long j2) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        long m2472toLongimpl = Duration.m2472toLongimpl(j2, unit);
        if (((j - 1) | 1) == Long.MAX_VALUE) {
            return m2531checkInfiniteSumDefinedPjuGub4(j, j2, m2472toLongimpl);
        }
        if ((1 | (m2472toLongimpl - 1)) == Long.MAX_VALUE) {
            return m2533saturatingAddInHalvesNuflL3o(j, unit, j2);
        }
        long j3 = j + m2472toLongimpl;
        return ((j ^ j3) & (m2472toLongimpl ^ j3)) < 0 ? j < 0 ? Long.MIN_VALUE : Long.MAX_VALUE : j3;
    }

    /* renamed from: checkInfiniteSumDefined-PjuGub4  reason: not valid java name */
    private static final long m2531checkInfiniteSumDefinedPjuGub4(long j, long j2, long j3) {
        if (!Duration.m2458isInfiniteimpl(j2) || (j ^ j3) >= 0) {
            return j;
        }
    ...
    private static final long m2533saturatingAddInHalvesNuflL3o(long j, DurationUnit durationUnit, long j2) {
        long m2436divUwyO8pc = Duration.m2436divUwyO8pc(j2, 2);
        long m2472toLongimpl = Duration.m2472toLongimpl(m2436divUwyO8pc, durationUnit);
        return (1 | (m2472toLongimpl - 1)) == Long.MAX_VALUE ? m2472toLongimpl : m2532saturatingAddNuflL3o(m2532saturatingAddNuflL3o(j, durationUnit, m2436divUwyO8pc), durationUnit, Duration.m2461minusLRDsOJo(j2, m2436divUwyO8pc));
    }

    private static final long infinityOfSign(long j) {
        return j < 0 ? Duration.Companion.m2523getNEG_INFINITEUwyO8pc$kotlin_stdlib() : Duration.Companion.m2522getINFINITEUwyO8pc();
    }

    public static final long saturatingDiff(long j, long j2, DurationUnit unit) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        if ((1 | (j2 - 1)) == Long.MAX_VALUE) {
            return Duration.m2477unaryMinusUwyO8pc(infinityOfSign(j2));
        }
    ...
    public static final long saturatingOriginsDiff(long j, long j2, DurationUnit unit) {
        ...
        if (((j2 - 1) | 1) == Long.MAX_VALUE) {
            if (j == j2) {
                return Duration.Companion.m2524getZEROUwyO8pc();
            }
            return Duration.m2477unaryMinusUwyO8pc(infinityOfSign(j2));
    ...
    private static final long saturatingFiniteDiff(long j, long j2, DurationUnit durationUnit) {
        ...
        if (((j3 ^ j) & (~(j3 ^ j2))) < 0) {
            if (durationUnit.compareTo(DurationUnit.MILLISECONDS) < 0) {
                ...
                return Duration.m2462plusLRDsOJo(DurationKt.toDuration((j / convertDurationUnit) - (j2 / convertDurationUnit), DurationUnit.MILLISECONDS), DurationKt.toDuration(j4, durationUnit));
            }
            return Duration.m2477unaryMinusUwyO8pc(infinityOfSign(j3));
        }
